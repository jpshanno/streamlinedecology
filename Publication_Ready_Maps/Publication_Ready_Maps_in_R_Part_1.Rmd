...
I had previously used the `sp` package for spatial data analysis and plotting. I was never completely stratisfied with how `sp` integrated with my regular workflow, especially after I began to more heavily use the tidyverse.  Then in Fall 2018 I came across the package `sf`, which makes working with data, especially tidy data, simple, most of your regular workflow carries right over. So for these maps I'll be using `sf` and other tidy packages. I stuck with `ggplot2` for the plotting because I am already familiar with the syntax and some of the quirks.
